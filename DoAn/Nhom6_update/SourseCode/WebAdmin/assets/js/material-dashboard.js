...
document.addEventListener("DOMContentLoaded", function() {
  var toastButtonList = document.querySelectorAll(".toast-btn");

  toastButtonList.forEach(function(btn) {
    btn.addEventListener("click", function() {
      ...
      const titleEl = toastEl.querySelector(".toast-header span.me-auto");
      if (newTitle && titleEl) {
        titleEl.textContent = newTitle;
      }

      // CUSTOM MESSAGE
      const newMessage = btn.dataset.message;
      const bodyEl = toastEl.querySelector(".toast-body");
      if (newMessage && bodyEl) {
        bodyEl.textContent = newMessage;
      }

      // CUSTOM ICON
      const newIcon = btn.dataset.icon;
      const iconEl = toastEl.querySelector(".toast-header i.material-symbols-rounded");
      if (newIcon && iconEl) {
...
function showToast(toastId, message = null, title = null, icon = null) {
  const template = document.getElementById(toastId);
  if (!template) return;
  ...
  clone.classList.remove("hide"); // Make sure it's not hidden
  clone.classList.add("show");    // Make it visible
  ...
  // Set title
  if (title) {
    const titleEl = clone.querySelector('.toast-header span.me-auto');
    if (titleEl) titleEl.textContent = title;
  }

  // Set icon
  if (icon) {
    const iconEl = clone.querySelector('.toast-header i.material-symbols-rounded');
